    ...
    <div className='border p-6 shadow rounded' >
        ...
        <h4 className='font-medium text-xl mt-2'>
            ...
            <div className='mt-4 flex gap-4 text-sm'>
                <button type="button" onClick={()=>{navigate(`/apply-job/${job._id}`); window.scrollTo(0,0)}} className='bg-blue-600 text-white px-4 py-2 rounded '>Apply now</button>
                <button type="button" onClick={()=>{navigate(`/apply-job/${job._id}`); window.scrollTo(0,0)}} className='text-gray-500 border border-gray-500 rounded px-4 py-2'> Learn more</button>
            </div>
