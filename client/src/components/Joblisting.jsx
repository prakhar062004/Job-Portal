...
const Joblisting = () => {
   ...
   useEffect(()=>{
    ...
    const matcheTitle = job => searchFilter.title === "" || job.title.toLowerCase().includes(searchFilter.title.toLowerCase());
    const matchesSearchLocation = job => searchFilter.location === "" || job.location.toLowerCase().includes(searchFilter.location.toLowerCase());

    const newFilteredJobs = jobs.slice().reverse().filter(
      job => matchesCategory(job) && matchesLocation(job) && matcheTitle(job) && matchesSearchLocation(job)
    )
  ...
  return (
    <div className='container mx-auto 2xl:px-20 flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            <div className='mb-4 text-gray-600'>
              {searchFilter.title && (
                <span className='inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 px-4 py-1.5 rounded'>
                  {searchFilter.title}
                  <img onClick={() => setSearchFilter(prev => ({...prev, title: ""}))} className='cursor-pointer' src={assets.cross_icon} alt="Remove title filter" />
                </span>
              )}
              {searchFilter.location && (
                <span className='ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>
                  {searchFilter.location}
                  <img onClick={() => setSearchFilter(prev => ({...prev, location: ""}))} className='cursor-pointer' src={assets.cross_icon} alt="Remove location filter" />
                </span>
              )}
            </div>
          </>
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mr-4'>
          {jobs && jobs.length > 0 ? (
