...
const Dashboard = () => {
   ...
   // Fix: use correct lowercase casing matching the context exports
   const { companyData, setcompanyData, setcompanyToken } = useContext(AppContext)

   // Function to log out from company
   const logout = () => {
    ...
    <div className='min-h-screen'>
        ...
        <div className='shadow py-4'>
            <div className='px-5 flex justify-between items-center'>
                <img onClick={() => navigate('/')} className='max-sm:w-32 cursor-pointer' src={assets.logo} alt="Job Portal Logo" />
                    {companyData && (
                         <div className='flex items-center gap-3'>
                    <p className='max-sm:hidden'>Welcome, {companyData.name}</p>
                    <div className='relative group'>
                       <img className='w-8 border rounded-full' src={companyData.image} alt="Company logo" />
                       <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-12 mt-2'>
